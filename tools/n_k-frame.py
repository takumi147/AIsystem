...
def calculate_f_score(file_pres=[], file_num=0, unit_size=16, a=0.5, b=0.5, conf_thre=0.8, iou_thre=0.5):
    """
        ...
        for i in range(unit_num+1):
            ...
            for j in range(1, unit_size+1):
                ...
                if os.path.exists(pre_txt_path):
                    pre_txt_conf = round(float(open(pre_txt_path).read().split(' ')[-1][:-2]), 3)
                    iou = get_iou(pre_txt_path, tru_txt_path)
                    if pre_txt_conf >= conf_thre and iou >= iou_thre:
                        flame_pre_num += 1
...
if __name__ == '__main__':
    file_pres = ['']
    file_num = 379
    print(calculate_f_score(file_pres, file_num, unit_size=16, a=0.5, b=0.5, conf_thre=0.8, iou_thre=0.5))
