...
2022/12/5
注意两个重要的东西，一个是数据格式统一，另一个是如果要迭代列表，要保证列表长度不能变。
1. iou对于k-frame是否有必要，前者评价模型，后者评价系统。如果从系统角度考虑，也就是说希望将被遮挡的手杖检测出来，以及屏蔽
不会动的误检测物体的话，也许只满足这两个条件才是系统的目的。假设一个人拿着雨伞到处走，被识别是白杖，那也是没有办法的，是模型
的问题不是系统的问题。